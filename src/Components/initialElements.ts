...
export const nodeData = {
  ...
  nodesData: function nodesData  () :Node[] {
    ...
    data.apiComponentList.map((d:any , idx:Number)=>{
      ...
      }else if(d.ruleConfigIdentifierDefinition)
      {
        arr.push({
          type: 'shape',
          id: index.toString(),
          data: {shape: 'diamond', height: 60,width: 160, label: d.ruleConfigIdentifierDefinition.name, color: '#eba834'},
          position: { x: 0, y: 0 },
        });
    ...
    if(data.responseDefinition)
    {
      ...
      arr.push({
        type: 'shape',
        id: index.toString(),
        data: {shape:'round-rect',height: 80, label: JSON.stringify(data.responseDefinition?.responseMapping, null, '\t').toString(), color: '#ebd934'},
        position: { x: 0, y: 0 },
      });
  ...
  edgesData: function edgesData  () :Edge[] {
    let arr1: Edge[] = [];
    let index = 0;
